...
让 AI 帮你做前端设计调研时，主动避开 5 类「AI 味」陷阱，并强制输出三份相互引用的文档（设计研究 / 生图提示词 / 实施 plan）。
...
把仓库里的 `SKILL.md` 复制到 Codex 的 skills 目录，或在 Codex Desktop 的自定义指令里引用本仓库的 raw URL：
...
WindSurf / Cline / Continue / Aider / Gemini CLI / GitHub Copilot 通用做法：把 `SKILL.md` 内容粘到该工具的 rules / system prompt / custom instructions 里，无需任何修改。本质就是一份结构化的方法论 markdown，任何能读 markdown 的 agent 都能用。
...
- `references/anti-ai-patterns.md` — 反陷阱完整清单 + 根因解释
...
- `examples/example-homepage.png` — 真实输出示例（编辑设计风）
- `examples/example-dashboard.png` — 真实输出示例（Dashboard 风）
...
**多工具协作是默认路径，不是补充。** 三份文档不只是给 Claude Code 用，它们会被进一步压缩成 v0 / bolt / lovable / Cursor 能消费的 prompt。Codex 在写代码时可以顺便批量产出这些 prompt。
